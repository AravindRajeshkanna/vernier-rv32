...
AMOs) and on both cores' instruction fetch. It has
...
- **PMP faults, either core's data path or instruction fetch** — a
  configured, matching PMP region failing to deny a load, store, AMO, or
  fetch it should (or denying one it should not), the lock bit not
  applying to M-mode, or a region's address match itself being wrong
  (`rtl/pmp.v`).

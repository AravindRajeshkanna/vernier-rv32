...
.PHONY: all sim wave wave_soc verilator software sim_software soc card ramimage probeimage \
        ...
        sim_sdram sim_sdramboot sdramimage sim_sdramprobe sim_sdramcheck sim_ddr3_init sim_ddr3_data sim_ddr3_top sim_ddr3_dqs_write sim_ddr3_write_seq sim_ddr3_read_seq sim_ddr3_read_burst_ext sim_ddr3_cmd_seq sim_ddr3_refresh_ctrl sim_ddr3_refresh_wire sim_ddr3_reverse_arb sim_ddr3_wr_excl sim_ddr3_model_banks sim_ddr3_addr sim_ddr3_phy_phases sim_ddr3_wr_window sim_ddr3_dq_window_rules synth_check_ddr3 \
        sim_jtag \
...
# every alignment of the request against the refresh deadline, for both a
...
sim_ddr3_reverse_arb: sim/sim_ddr3_reverse_arb.out
	@cd sim && $(VVP) sim_ddr3_reverse_arb.out $(VVP_DUMP) 2>&1 | tee ddr3_reverse_arb.log
	@grep -q "DDR3 REVERSE ARBITRATION TEST PASSED" sim/ddr3_reverse_arb.log || \
...
# against the real command pins; swept over every offset of a second
...
sim_ddr3_wr_excl: sim/sim_ddr3_wr_excl.out
	@cd sim && $(VVP) sim_ddr3_wr_excl.out $(VVP_DUMP) 2>&1 | tee ddr3_wr_excl.log
...
# The survey measured DQ enabled for one cycle, one cycle before DQS was even
# enabled; with the pin-based checker the same design could not complete
...
# ---- DDR3 write-burst rules, self-test (Phase 9 Stage 1, Part 17) ----
#
# sim/ddr3_dq_model.v judges a write burst from the pins against the DRAM's own
# write latency. A checker whose rules cannot be shown to fire is not a checker,
# and the aligned design never breaks them - so each rule gets a directed stream
# on its own model instance, plus legal controls so a too-strict rule fails here.
sim/sim_ddr3_dq_window_rules.out: sim/tb_ddr3_dq_window_rules.v rtl/soc/ddr3_eclk_pll.v sim/ddr3_dq_model.v
	$(IVERILOG) $(IVFLAGS) -o $@ sim/tb_ddr3_dq_window_rules.v rtl/soc/ddr3_eclk_pll.v sim/ddr3_dq_model.v

sim_ddr3_dq_window_rules: sim/sim_ddr3_dq_window_rules.out

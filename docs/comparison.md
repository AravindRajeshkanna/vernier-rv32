...
- **Video output is unbuilt; a Network-on-Chip interconnect is a plan
  ...
  adopted, and twelve real, gated, mutation-tested PHY slices exist in
  simulation, including a first real command-driven read/write round
  trip and refresh arbitrated against write/read in both directions;
  real hardware bring-up is what remains open, not a board decision.

...
- **Video output is unbuilt; a Network-on-Chip interconnect is a plan
  ...
  adopted, and seventeen real, gated, mutation-tested PHY slices exist in
  ...
  bank, a memory model that decodes the address the way the part
  does, CK at the edge-clock rate as Lattice's own reference design
  does, and a write burst that lands where the DRAM's write latency says -

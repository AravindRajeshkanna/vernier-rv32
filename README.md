...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | **Stages 0-1 done, in simulation** — a new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing (interconnect, reservation ports, bus adapter) with no changes needed to any of it. Proven both by the existing cross-hart smoke test (reused unchanged) and a genuine module-identity check (hart 1's own `rob_count` register, confirmed non-vacuous by mutation), with zero regression on every existing single-core and homogeneous-multi-core build. Still open: cross-hart LR/SC coherence, boot firmware/device-tree/OpenSBI awareness, and Linux SMP on the mixed pair — none of that is attempted yet. Full account in `docs/roadmap.md` |

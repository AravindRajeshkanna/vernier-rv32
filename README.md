...
| 9 | DDR | **no longer blocked — real board and real PHY work underway.** A second board target, ECPIX-5 (Lattice ECP5-5G `LFE5UM5G-85F`, 512 MB DDR3L), was confirmed feasible and adopted; a custom, hand-written DDR3 PHY/controller was chosen over adapting an existing one (LiteDRAM: generated, not hand-written; UberDDR3: GPL-3.0, license-incompatible). Stage 0 (board bring-up, no DDR yet): `fpga/ecpix5_top.v` and real, cited pin constraints exist and synthesize — 25.91 MHz PASS at a diagnostic framebuffer scale; the real 320×240 scale attempt was killed by this machine's own 8 GB memory limit after ~7.5 hours, genuinely unconfirmed either way. Stage 1 (the PHY itself), nine real, gated, mutation-tested slices in, closing with the first real, command-driven write-then-read round trip: a real JEDEC init/mode-register sequence against a real protocol checker (DLL-off mode, CL=CWL=6); one byte lane's own `DQSBUFM`-based DQ/DQS data path with a real, bounded `READCLKSEL` calibration sweep (LiteDRAM's real architecture, not UberDDR3's simpler one — the user's own explicit choice); the init sequence and the DQ/DQS path unified onto one real, shared, PLL-derived clock tree; a real DQS write-drive primitive, proven standalone then wired in, making the DQ/DQS pins genuinely bidirectional; real ACT+WR and ACT+RD command sequencers with measured (not assumed) tRCD/CWL/CL timing; a real extender closing the shape mismatch between those sequencers' own single-cycle pulses and the DQ/DQS path's own level-held capture window; and both sequencers wired into the top-level PHY through four real, construction-mutually-exclusive muxes, proving a genuine ACT+WR then ACT+RD round trip returns the exact byte written — not calibration's own direct-injection scheme. Several real bugs were found this way and are worth naming plainly: a clock-generation construction that hand-tracing said was correct and a real simulation run showed was not; a bus-contention check that tapped the wrong internal signal and missed a real fault a mutation exposed; a reduction-XOR check that misreported a floating bus as contended; a test stimulus race (blocking assignment against a DUT's own NBA-clocked register) that briefly looked like a real design bug and was not; and a `read_data_valid` gate whose first fix still sampled one cycle too early, caught only by re-tracing a real probe a second time, not assumed fixed after the first attempt. Still open, named rather than implied done: the memory model is still a single stored location, not address-decoded, the second DQ byte lane doesn't exist, and no ECPIX-5 hardware has been touched this session. Full account in `docs/roadmap.md` |
...
preserve. Phase 8 sits where it does because nothing else is blocked by it —
it waits on a board with a PCIe connector and SerDes, not on any work in
this repo. Phase 9 no longer waits on a board at all — a real target
(ECPIX-5) is adopted and real DDR3 PHY work is underway in simulation; only
actually holding that board is still pending. `docs/roadmap.md` is specific,
phase by phase, about what each one requires and what must not regress
while it happens.

...
| 14 | Neural processing — quantized inference | ✅ **"Done when" closed, in simulation, at this scale** — a memory-mapped peripheral (`rtl/soc/wb_npu.v`, not a core instruction), a fixed 16-element int8 MAC engine, running a real small quantized dense layer (a 4-neuron, 16-input int8 matrix-vector multiply) verified bit-exact against an independently-computed Python reference and measured, not estimated, against a software-only (RV32M-only) baseline computing the identical workload: 561 cycles on the NPU versus 652 in plain C — real, honestly-counted, and modest, not the order-of-magnitude a "hardware acceleration" framing might suggest. The bus-mastering DMA port named as future work is now built too — a real Wishbone master (`rtl/soc/wb_ptw.v`'s own role, lowest arbitration priority, `formal/fv_interconnect.v` proves it correctly), reaching real RAM through the real interconnect, no longer capped at the MMIO path's fixed 16 elements — and measured at that larger scale (128 inputs, 4 neurons): **1220 cycles on the NPU versus 4838 in plain C — about 4.0x**, a real and substantial win this time, a useful contrast with the MMIO-scale result above. Still open: real trained-model weights rather than synthetic data, and confirmation on real hardware. Full accounting in `docs/roadmap.md` |

...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | **Stages 0-2 done, in simulation** — a new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing (interconnect, reservation ports, bus adapter) with no changes needed to any of it, and cross-hart LR/SC coherence is proven in both directions — an in-order hart's own reservation invalidated by an out-of-order hart's write, and the mirror image — both passing on the first attempt, no new coherence bug found. Every claim backed by a genuine module-identity check (hart 1's own `rob_count` register) and, for the coherence claim specifically, a targeted mutation confirmed non-vacuous, with zero regression on every existing single-core and homogeneous-multi-core build. Still open: boot firmware/device-tree/OpenSBI awareness of a mixed pair, Linux SMP on it, and measurement — none of that is attempted yet. Full account in `docs/roadmap.md` |

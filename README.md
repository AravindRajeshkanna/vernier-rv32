...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | not started — a plan only. `CORE=inorder`/`CORE=ooo` is a global, compile-time choice today (one core type per build, every hart); this phase's own new work is making that a per-hart choice, then re-proving cross-hart coherence, boot and SMP Linux for a genuinely asymmetric pair rather than the two identical-type pairs Phase 13 already closed. Full plan in `docs/roadmap.md` |

...
| 8 | Network-on-Chip interconnect | **not started, a plan only** — not blocked on hardware; a real, measured decision on whether to evolve `rtl/soc/wb_interconnect.v`'s own shared-bus arbitration into a real network fabric, gated on whether real contention actually shows up under 4+ harts or several high-bandwidth accelerators, not built ahead of that evidence. Full plan (six stages: measure, network interfaces, a minimal router, scaling/QoS, software transparency, real hardware with a build-time `INTERCONNECT=bus`/`INTERCONNECT=noc` choice) in `docs/roadmap.md` |
| 9 | DDR | **no longer blocked — real board and real PHY work underway.** A second board target, ECPIX-5 (Lattice ECP5-5G `LFE5UM5G-85F`, 512 MB DDR3L), was confirmed feasible and adopted; a custom, hand-written DDR3 PHY/controller was chosen over adapting an existing one (LiteDRAM: generated, not hand-written; UberDDR3: GPL-3.0, license-incompatible). Stage 0 (board bring-up, no DDR yet): `fpga/ecpix5_top.v` and real, cited pin constraints exist and synthesize — 25.91 MHz PASS at a diagnostic framebuffer scale; the real 320×240 scale attempt was killed by this machine's own 8 GB memory limit after ~7.5 hours, genuinely unconfirmed either way. Stage 1 (the PHY itself), seventeen real, gated, mutation-tested slices in, closing with a write burst that lands where the DRAM's write latency says, on top of CK at the edge-clock rate (twice `sclk`) with two command slots per `sclk`, an address-decoded memory model, real PRECHARGE after every transaction and two-directional refresh arbitration and at-most-one-transaction-in-flight, all wired into the top-level PHY: a real JEDEC init/mode-register sequence against a real protocol checker (DLL-off mode, CL=CWL=6); one byte lane's own `DQSBUFM`-based DQ/DQS data path with a real, bounded `READCLKSEL` calibration sweep (LiteDRAM's real architecture, not UberDDR3's simpler one — the user's own explicit choice); the init sequence and the DQ/DQS path unified onto one real, shared, PLL-derived clock tree; a real DQS write-drive primitive, proven standalone then wired in, making the DQ/DQS pins genuinely bidirectional; real ACT+WR and ACT+RD command sequencers with measured (not assumed) tRCD/CWL/CL timing; a real extender closing the shape mismatch between those sequencers' own single-cycle pulses and the DQ/DQS path's own level-held capture window; both sequencers wired into the top-level PHY through four real, construction-mutually-exclusive muxes, proving a genuine ACT+WR then ACT+RD round trip returns the exact byte written — not calibration's own direct-injection scheme; a real refresh scheduler with **the first primary-datasheet-verified DDR3 timing value in this whole investigation** (Micron's own real `MT41K256M16` datasheet, the chip a targeted search identifies as ECPIX-5's own — tREFI = 7.8125µs, tRFC = 260ns for its 4Gb density, cross-checked against a second real manufacturer's 2Gb part); that scheduler wired in, with refresh waiting for an in-flight write or read; and the reverse direction, a new request held off while a refresh is pending or running (Micron's own datasheet: "only NOP and DES commands are allowed after a REFRESH command and until tRFC (MIN) is satisfied"), resolved without dropping or queuing a request by making the caller-visible `busy` rise one cycle before the gate closes; and every request now either fully accepted or fully ignored, counted against the real command pins across every offset of a second request (a write and a read in flight together used to collide silently); and every transaction closing its own bank with a PRECHARGE, checked by a simulation model that now tracks per-bank state and whose own rules are each proven able to fire; and a memory model that decodes bank, row and column the way the part does (Micron's Table 2 for the x16 device), so a write landing in the wrong cell is finally visible - three of four address-path bugs it was proven against passed every earlier DDR3 test; and the clock structure of Lattice's own reference DDR3 write side (read from Lattice's FPGA-TN-02035 this time), found necessary by measuring CK at exactly 1.00 times `sclk` against a data path that moves four UI per `sclk`, and now measured at 2.00 - and, judging writes from the DQ and DQS pins instead of an internal tap, a memory model that could not complete calibration against the unaligned design - the write path had never worked at that fidelity - with the burst now driven exactly while DQS toggles, WL after the WRITE, and the first tool ever to read the synthesis half of the PHY (`make synth_check_ddr3`, which elaborates every ECP5 primitive against yosys's own cell library and was shown able to fail). Several real bugs were found this way and are worth naming plainly: a clock-generation construction that hand-tracing said was correct and a real simulation run showed was not; a bus-contention check that tapped the wrong internal signal and missed a real fault a mutation exposed; a reduction-XOR check that misreported a floating bus as contended; a test stimulus race (blocking assignment against a DUT's own NBA-clocked register) that briefly looked like a real design bug and was not; a `read_data_valid` gate whose first fix still sampled one cycle too early, caught only by re-tracing a real probe a second time; a refresh-scheduler test whose own two failures on first run both traced back to the test's own measurement reference points, not the design, confirmed by yet another real probe; and, wiring refresh in, a test whose first design ran a long write stream and merely hoped a collision would happen — it did, yet a mutation that completely broke the grant logic still passed clean, since two deterministic schedules not colliding by chance proves nothing, fixed by deliberately engineering the collision instead; and, closing the reverse direction, a sweep test that passed on its first run and so proved nothing until seven targeted mutations were each caught, one of which measured a real race in the naive design rather than assuming it; and, closing the write/read gap, a sweep written and run against the unfixed design first, which found a stale-data bug hiding since Part 9, reachable only by a caller that breaks the busy contract; and, closing the PRECHARGE gap, a model given bank state that showed Part 9's own headline write-then-read round trip had been an illegal command sequence all along (its read activated a bank the write never closed). Still open, named rather than implied done: the controller opens and closes a bank around every access rather than exploiting open rows (a transaction is now ~22 cycles for a write, ~18 for a read), tRCD/tRAS/tRP/tRC are not modelled because each is under one cycle at 25 MHz, the data path is not yet hardware-faithful (the memory model has no DRAM READ latency, so a wrong CL is invisible to every integrated test; all eight beats of a burst carry one byte - see `docs/roadmap.md`'s Phase 9 survey and Parts 16 and 17), the second DQ byte lane and the data-mask pins don't exist (the part is x16, the data path is one byte), this stage has never run under Verilator (only Icarus), and no ECPIX-5 hardware has been touched this session. Full account in `docs/roadmap.md` |
...
| 11 | DSP | ✅ **"Done when" closed, in simulation, at this scale** — a dedicated coprocessor (`rtl/soc/wb_fir.v`, not a core ISA extension), an 8-tap streaming FIR filter, running a real windowed-sinc lowpass filter over a 128-sample synthetic signal, verified bit-exact against an independently-computed Python reference and measured, not estimated, against the RV32M-only software baseline computing the identical filter: **3553 cycles on the FIR peripheral versus 17993 in plain C — about 5.1x**, a real and substantial win, unlike Phase 14's own NPU result (~1.16x) — different workload shapes give genuinely different answers. Still open: a real recorded signal rather than synthetic data, and confirmation on real hardware. Full accounting in `docs/roadmap.md` |
...
| 13 | Multi-core: both cores, one SoC | ✅ **every "Done when" clause closed, for the in-order core, in simulation** — `rtl/soc/soc_top.v` builds and runs two harts (`NUM_HARTS=2`), cross-hart LR/SC is coherent (a directed test proves a real hazard), both harts are visible to OpenSBI (`Platform HART Count : 2`) and to a `CONFIG_SMP=y` Linux that reaches userspace on both (`/proc/cpuinfo` shows harts 0 and 1), and the boot ROM parks a second hart and hands it a correct `mhartid`, and now a real device tree too (`a1` no longer a literal 0), through the real boot path. `rtl/debug/dm.v`'s own `hartsel` now reaches every hart too, with a real cross-hart register-isolation proof, not just hart 0. `CORE=ooo` now has real cross-hart LR/SC coherence too (the same reservation ports `cpu_core.v` has, wired to a real monitor and proven with the same cross-hart hazard test), and boots the same SMP Linux to userspace on both harts, on the first attempt with no new coherence bug found. `CORE=ooo` also gained the same halt/resume/single-step/register-access ports `cpu_core.v` has, now wired to a real debug module the same way `cpu_core.v`'s own are, with the real DMI/JTAG path proven end-to-end against `core_ooo.v` too, including cross-hart isolation. No board build has ever asked for a second hart — nothing outside simulation can reach one yet |
| 14 | Neural processing — quantized inference | ✅ **"Done when" closed, in simulation, at this scale** — a memory-mapped peripheral (`rtl/soc/wb_npu.v`, not a core instruction), a fixed 16-element int8 MAC engine, running a real small quantized dense layer (a 4-neuron, 16-input int8 matrix-vector multiply) verified bit-exact against an independently-computed Python reference and measured, not estimated, against a software-only (RV32M-only) baseline computing the identical workload: 561 cycles on the NPU versus 654 in plain C — real, honestly-counted, and modest, not the order-of-magnitude a "hardware acceleration" framing might suggest. The bus-mastering DMA port named as future work is now built too — a real Wishbone master (`rtl/soc/wb_ptw.v`'s own role, lowest arbitration priority, `formal/fv_interconnect.v` proves it correctly), reaching real RAM through the real interconnect, no longer capped at the MMIO path's fixed 16 elements — and measured at that larger scale (128 inputs, 4 neurons): **1220 cycles on the NPU versus 4840 in plain C — about 4.0x**, a real and substantial win this time, a useful contrast with the MMIO-scale result above. A bounded on-chip cache for the shared activation vector (`A_CACHE_LEN`, default 128) closes the specific inefficiency that measurement named — re-fetching the same vector once per neuron — bringing the DMA-scale workload from 1220 to **986 cycles (about 1.24x faster than the uncached DMA path, ~19%)**, confirmed through the real interconnect and confirmed non-vacuous by mutation. Real trained-model weights close another named gap: `software/soc/gen_npu_trained_layer.py` trains a from-scratch linear softmax classifier by gradient descent on a real cross-entropy loss (88.0% held-out accuracy, measured over 100 fresh examples) — genuinely trained weights, not `random.Random` draws, quantized to int8 by standard post-training quantization. The hardware reproduces the trained float model's own classification decision on a held-out input, not just a matching dot product. Still open: the training data itself is synthetic, not real-world, and confirmation on real hardware. Full accounting in `docs/roadmap.md` |
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | ✅ **"Done when" closed, in simulation, at this scale** — every criterion that bar names now has a real citation: one elaboration containing both hart types at once (module-identity mutation-confirmed), cross-hart LR/SC *and* ordinary loads/stores both proven correct under the same directed-hazard standard (LR/SC in both directions; ordinary loads/stores via a dedicated new test closing the one gap this phase's own account had left open — no such test existed for any pairing, homogeneous included, before it — confirmed against a real mutated interconnect, not just a testbench-level check), OpenSBI/`CONFIG_SMP=y` Linux booting both hart types to userspace on the first attempt, and zero regression on any existing configuration. A new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing with no changes needed to any of it. The real boot-ROM mailbox holds under real asymmetry, and CoreMark runs concurrently on both harts at once (a new dual-hart harness, since nothing like it existed before) — `cpu_core.v` alone: 419,621 cycles; `core_ooo.v` alone: 360,481 cycles; the mixed pair: 508,725/463,080 cycles per hart, landing between the two homogeneous pairs' own combined throughput, exactly as expected from mixing a faster core with a slower one. Building that harness surfaced a real, previously-undiscovered defect — plain AMOs had no working cross-hart atomicity anywhere in this SoC, formally "proved" since Phase 13 but never actually true against real hardware timing — found, root-caused, and fixed as its own standalone PR (`docs/roadmap.md`'s Phase 13 entry, corrected in place) before this stage could even produce a number. Stage 3's other half — giving each `cpu@` node a real per-microarchitecture `compatible` string — needed a `$(CORE)`-suffixed device-tree build pipeline rather than a two-line fix, and that pipeline is built and gated: `dts/soc_$(CORE).dtb` → `dtb_blob_$(CORE).h` → `bootrom_$(CORE).elf`/`sim/bootrom_$(CORE).hex`, confirmed live in a real Linux boot's own `/proc/cpuinfo`. Not required by this bar's own wording, and still open: confirmation on real hardware — no board build has ever asked for `NUM_HARTS>1` of any kind. Full account in `docs/roadmap.md` |
...
preserve. Phase 8 waits on nothing at all — only on whether a real
measurement of `rtl/soc/wb_interconnect.v`'s own contention under
multi-master load ever justifies building a Network-on-Chip fabric. Phase 9
no longer waits on a board at all — a real target (ECPIX-5) is adopted and
real DDR3 PHY work is underway in simulation; only actually holding that
board is still pending. `docs/roadmap.md` is specific, phase by phase, about
what each one requires and what must not regress
while it happens.

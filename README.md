...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | **Stages 0-2 done, plus half of Stage 3, in simulation** — a new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing (interconnect, reservation ports, bus adapter, and now the real boot-ROM mailbox too) with no changes needed to any of it. Cross-hart LR/SC coherence is proven in both directions — an in-order hart's own reservation invalidated by an out-of-order hart's write, and the mirror image — and the real boot-ROM mailbox (`crt0_rom.S`'s park/release handshake) is proven to hold when the parked hart is the opposite microarchitecture from the one that released it — all three passing on the first attempt, no new bug found. Every claim backed by a genuine module-identity check (hart 1's own `rob_count` register) and, for the coherence claim specifically, a targeted mutation confirmed non-vacuous, with zero regression on every existing single-core and homogeneous-multi-core build. Stage 3's other half — giving each `cpu@` node a real per-microarchitecture `compatible` string — turned out to need a `CORE`-suffixed device-tree build pipeline, not a two-line fix, and is deferred with the reasoning on record rather than shipped as a shortcut. Still open: that device-tree work, OpenSBI/Linux SMP awareness of a mixed pair, and measurement. Full account in `docs/roadmap.md` |

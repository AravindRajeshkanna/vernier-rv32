...
| 11 | DSP | **stage 1** — the direction is picked (a dedicated coprocessor, `rtl/soc/wb_fir.v`, not a core ISA extension, matching the NPU's own precedent), and an 8-tap streaming FIR filter proves the mechanism: a sliding-window delay line shifted on every new sample, and a saturating (not wrapping) 40-bit accumulator for overflow an int8 engine never had to handle. Not yet done: a real filtering workload, and the soft-math baseline this phase's own "Done when" bar asks be measured first. Full accounting in `docs/roadmap.md` |

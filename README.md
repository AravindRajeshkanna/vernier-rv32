...
| 14 | Neural processing — quantized inference | **stage 1 of an undetermined number** — the ISA-vs-MMIO design question is closed (a memory-mapped peripheral, `rtl/soc/wb_npu.v`, not a core instruction, matching the blit engine's own precedent), and a fixed-depth (16-element) int8 dot-product engine proves the mechanism: MMIO-loaded operands, a sequential MAC datapath, verified against an independently-computed reference both standalone and through the real CPU load/store path. Nowhere near the actual "Done when" bar yet — no bus-mastering DMA, no real inference workload, nothing measured against a software-only baseline. Full accounting in `docs/roadmap.md` |

...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | **Stages 0-2 and 4 done, plus half of Stage 3, in simulation** — a new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing (interconnect, reservation ports, bus adapter, boot-ROM mailbox, and now OpenSBI/Linux boot too) with no changes needed to any of it. Cross-hart LR/SC coherence is proven in both directions, the real boot-ROM mailbox is proven to hold when the parked hart is the opposite microarchitecture from the one that released it, and — with zero code changes at all, since the existing `sim_opensbi_2hart`/`sim_linux_2hart` targets were already `$(CORE)`-generic — OpenSBI detects both harts and the unmodified `CONFIG_SMP=y` kernel boots both to userspace (`smp: Brought up 1 node, 2 CPUs`), all passing on the first attempt with no scheduler warning, RCU stall, or other symptom beyond ordinary cycle-count noise. Every claim backed by a genuine module-identity check (hart 1's own `rob_count` register) and, for the coherence claim specifically, a targeted mutation confirmed non-vacuous, with zero regression on every existing single-core and homogeneous-multi-core build. Stage 3's other half — giving each `cpu@` node a real per-microarchitecture `compatible` string — turned out to need a `CORE`-suffixed device-tree build pipeline, not a two-line fix, and is deferred with the reasoning on record. Still open: that device-tree work, and a real, quantified measurement of the throughput asymmetry (Stage 4 only checked pass/fail, not how much). Full account in `docs/roadmap.md` |

...
| 15 | Heterogeneous multi-core — both core types, one SoC, at once | **Stages 0 through 5 all done, in simulation** — a new `CORE=hetero` build genuinely elaborates hart 0 as `cpu_core.v` and every hart from 1 upward as `rtl/ooo/core_ooo.v`, reusing Phase 13's existing multi-hart plumbing with no changes needed to any of it. Cross-hart LR/SC coherence is proven in both directions, the real boot-ROM mailbox holds under real asymmetry, OpenSBI/Linux boot both harts to userspace with zero code changes, and CoreMark now runs concurrently on both harts at once (a new dual-hart harness, since nothing like it existed before) — `cpu_core.v` alone: 419,621 cycles; `core_ooo.v` alone: 360,481 cycles; the mixed pair: 508,725/463,080 cycles per hart, landing between the two homogeneous pairs' own combined throughput, exactly as expected from mixing a faster core with a slower one. Building that harness surfaced a real, previously-undiscovered defect — plain AMOs had no working cross-hart atomicity anywhere in this SoC, formally "proved" since Phase 13 but never actually true against real hardware timing — found, root-caused, and fixed as its own standalone PR (`docs/roadmap.md`'s Phase 13 entry, corrected in place) before this stage could even produce a number. Every claim backed by a genuine module-identity check and, where relevant, a targeted mutation confirmed non-vacuous, with zero regression on every existing configuration. Stage 3's other half — giving each `cpu@` node a real per-microarchitecture `compatible` string — did turn out to need a `$(CORE)`-suffixed device-tree build pipeline rather than a two-line fix, and that pipeline is now built and gated: `dts/soc_$(CORE).dtb` → `dtb_blob_$(CORE).h` → `bootrom_$(CORE).elf`/`sim/bootrom_$(CORE).hex`, confirmed live in a real Linux boot's own `/proc/cpuinfo`. Full account in `docs/roadmap.md` |

...
| 14 | Neural processing — quantized inference | ✅ **"Done when" closed, in simulation, at this scale** — a memory-mapped peripheral (`rtl/soc/wb_npu.v`, not a core instruction), a fixed 16-element int8 MAC engine, running a real small quantized dense layer (a 4-neuron, 16-input int8 matrix-vector multiply) verified bit-exact against an independently-computed Python reference and measured, not estimated, against a software-only (RV32M-only) baseline computing the identical workload: 561 cycles on the NPU versus 652 in plain C — real, honestly-counted, and modest, not the order-of-magnitude a "hardware acceleration" framing might suggest. Still open: real trained-model weights rather than synthetic data, a workload larger than fits in always-resident MMIO registers (needs the bus-mastering DMA port named but not yet built), and confirmation on real hardware. Full accounting in `docs/roadmap.md` |
